...
export const geminiService = {
  ...
  async getChatResponse(prompt: string, userName?: string) {
    const ai = getAI();
    const userContext = userName ? `You are addressing ${userName}. Use their name naturally in the conversation to build rapport.` : '';

    const response = await ai.models.generateContent({
      model: 'gemini-3-flash-preview',
      contents: `You are a highly capable multilingual cybersecurity assistant. ${userContext}
  ...
  async generateQuiz(difficulty: 'easy' | 'hard' = 'easy') {
    ...
    const difficultyContext = difficulty === 'hard' 
      ? 'ADVANCED technical MCQ for experts.'
      : 'SIMPLE digital safety MCQ for beginners.';
